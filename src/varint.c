...
void match_check(uint32_t* com, uint32_t* uncom) // проверка на совпадения значений в двух файлах
{
...
size_t generate() // генерирует и записывает числа в 2 бинарных файла (в сжатом и несжатом форматах)
{
    ...
    FILE* f;
    FILE *z;
    f = fopen("uncompressed.dat", "w"); // создаем файл для записи чисел в несжатом формате
    z = fopen("compressed.dat", "w"); // создаем файл для записи чисел в сжатом формате
    for (size_t i = 0; i < SIZE; ++i) 
    {
...
void filesize() //выводит размер файлов 
{
    ...
    size_of_file = ftell(f);//считываем местоположение указателя от начала файла (возвращается количество байт) 
